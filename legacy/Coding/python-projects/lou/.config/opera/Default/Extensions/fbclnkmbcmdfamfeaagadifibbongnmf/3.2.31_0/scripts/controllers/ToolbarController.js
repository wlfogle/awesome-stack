...
angular.module('myjdWebextensionApp')
    .controller('ToolbarCtrl', ['$q', '$rootScope', '$scope', '$timeout', '$interval', 'RequestQueueEventService', 'CnlService', 'ApiErrorService', 'StorageService', 'BackgroundScriptService', function ($q, $rootScope, $scope, $timeout, $interval, RequestQueueEventService, cnlService, apiErrorService, storageService, BackgroundScriptService) {
        $rootScope.bodyClass = "";
        ...
        function initSession() {
            BackgroundScriptService.getSessionInfo(instanceId).then(function (result) {
                $timeout(function () {
        ...
        function startTimeout() {
            ...
            addLinksDialogTimeout = $interval(function () {
                $timeout(function () {
                    if ($scope.state.countdown !== -1) {
                        $scope.state.countdown--;
                        if ($scope.state.countdown === 0) {
                            console.log($scope.state);
                            $scope.$broadcast(RequestQueueEventService.Events.COUNTDOWN_FINISHED_SEND_LINKS, {});
                        }
                    }
                }, 0);
            }, 1000, $scope.countdownOptions.seconds);
        }
        ...
        function init() {
            ...
            storageService.get(storageService.STORAGE_DEVICE_LIST_KEY, function (result) {
                $timeout(function () {
                    ...
                    if (!$scope.selection.device && $scope.devices && $scope.devices.length > 0) {
                        storageService.get([storageService.settingsKeys.DEFAULT_PREFERRED_JD.key, storageService.ADD_LINK_CACHED_OPTIONS], function (result) {
                            if (result[storageService.settingsKeys.DEFAULT_PREFERRED_JD.key]) {
                                if (result[storageService.ADD_LINK_CACHED_OPTIONS]) {
                                    $scope.selection.previousDevice = result[storageService.ADD_LINK_CACHED_OPTIONS].device;
                                }
                                $scope.selection.device = autoSelectDevice(result[storageService.settingsKeys.DEFAULT_PREFERRED_JD.key], storageService.AskEveryTimeDevice);
                            } else {
            ...
            storageService.getSettings(function (settings) {
                $scope.addLinksDialogActive = settings[storageService.settingsKeys.ADD_LINKS_DIALOG_ACTIVE.key] === undefined ? storageService.settingsKeys.ADD_LINKS_DIALOG_ACTIVE.defaultValue : settings[storageService.settingsKeys.ADD_LINKS_DIALOG_ACTIVE.key];
                $scope.countdownOptions.seconds = settings[storageService.settingsKeys.COUNTDOWN_VALUE.key] === undefined ? storageService.settingsKeys.COUNTDOWN_VALUE.defaultValue : settings[storageService.settingsKeys.COUNTDOWN_VALUE.key];
                $scope.countdownOptions.active = settings[storageService.settingsKeys.COUNTDOWN_ACTIVE.key] === undefined ? storageService.settingsKeys.COUNTDOWN_ACTIVE.defaultValue : settings[storageService.settingsKeys.COUNTDOWN_ACTIVE.key];
                updateLinks();
            });
        }
        ...
        $scope.selectDevice = function (device) {
            ...
            if (device !== 'EXTENSION_LINK_QUEUE' && $scope.cachedHistory && $scope.cachedHistory[$scope.selection.device.id]) {
        ...
        $scope.$on(RequestQueueEventService.Events.ADD_LINKS_DIALOG_CLOSE, function (event) {
            $timeout(function () {
                ...
                $timeout(function () {
                    ...
                    closeDialog();
                    if ($scope.isPopup) {
                        chrome.runtime.sendMessage({name: "close-me", data: {tabId: instanceId}});
                    }
                }, 800);
            }, 0);
        });

        $scope.$on(RequestQueueEventService.Events.HIDE_REQUEST_QUEUE, function (event) {
            $timeout(function () {
        ...
        function closeDialog() {
            ...
            if (window.parent == window && chrome.runtime.getURL("toolbar.html").indexOf(window.location.origin) !== -1) {
        ...
        function sendToExtensionLinkQeue(callback) {
            storageService.get(storageService.CLIPBOARD_HISTORY, function (result) {
                if (!result.CLIPBOARD_HISTORY) {
                    result.CLIPBOARD_HISTORY = {};
                }

                $.each($scope.state.requestQueue, function (index, value) {
                    result.CLIPBOARD_HISTORY[value.id] = value;
                });

                storageService.set(storageService.CLIPBOARD_HISTORY, result.CLIPBOARD_HISTORY, function (success) {
                    if (callback) {
                        callback();
                    }
                });
        ...
        function refreshDevices() {
            BackgroundScriptService.getDevices().then(function (result) {
                $timeout(function () {
                    ...
                    invalidateInitState();
                    $scope.selection.device = autoSelectDevice($scope.devices[0], storageService.AskEveryTimeDevice);
                }, 0);
            }).catch(function () {
                $timeout(function () {
                    $scope.errorMessage = "Failed to get a list of connected JDownloaders";
                }, 0);
            });
        }
        ...
        chrome.runtime.onMessage.addListener(function (msg) {
            if (msg) {
                if (msg.action && msg.action === "link-info-update" && msg.tabId && msg.tabId == instanceId) {
                    updateLinks();
                    $timeout(function () {
                        $scope.initState.links = true;
                        invalidateInitState();
        ...
        var autoSelectDevice = function (preferredDevice, fallbackDevice) {
            var deviceIdToFind;
            if (preferredDevice) {
                if (preferredDevice.id === storageService.LastUsedDevice.id && $scope.selection.previousDevice) {
                    deviceIdToFind = $scope.selection.previousDevice.id;
                } else if (preferredDevice.id === storageService.SaveForLaterDevice.id) {
                    return storageService.SaveForLaterDevice;
                } else if (preferredDevice.id === storageService.AskEveryTimeDevice.id) {
                    $scope.editMode = true;
