...
PackageInfo PackageManager::parsePackageEntry(const QString &entry) const
{
    ...
    if (parts.size() >= 2) {
        ...
        // Get package description using pacman -Qi
        QString infoOutput = runPacmanCommand({"-Qi", pkg.name});
        QStringList infoLines = infoOutput.split('\n');
        
        for (const QString &line : infoLines) {
            if (line.startsWith("Description")) {
                QRegularExpression re("Description\\s*:\\s*(.+)");
                QRegularExpressionMatch match = re.match(line);
                if (match.hasMatch()) {
                    pkg.description = match.captured(1).trimmed();
                }
                break;
            }
            if (line.startsWith("Installed Size")) {
                QRegularExpression re("Installed Size\\s*:\\s*(.+)");
                QRegularExpressionMatch match = re.match(line);
                if (match.hasMatch()) {
                    pkg.size = parseSize(match.captured(1).trimmed());
                }
            }
        }
        ...
        if (pkg.description.isEmpty()) {
            pkg.description = "Package: " + pkg.name;
        }
