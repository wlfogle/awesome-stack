#!/usr/bin/env python3
"""
HIDraw2 Monitor - Test device data
"""
import os
import sys
import time
...
def monitor_hidraw2():
    """Monitor /dev/hidraw2 for device data"""
    device_path = "/dev/hidraw2"
    
    print(f"🔍 Monitoring {device_path} for activity...")
...
if __name__ == "__main__":
    print("🔧 HIDraw2 Monitor")
    print("=" * 30)
    
    # Check if device exists
    if not os.path.exists("/dev/hidraw2"):
        print("❌ /dev/hidraw2 does not exist")
        sys.exit(1)
    
    # Check permissions
    if not os.access("/dev/hidraw2", os.R_OK):
        print("⚠️  No read permission for /dev/hidraw2")
