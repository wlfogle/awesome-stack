...
def checkargs():
    script_name = "qBittorrent Port Update (LXC Version)"
    version = "2.1.3-lxc"
    if len(sys.argv) !=1:
        if sys.argv[1] == "--help" or sys.argv[1] == "-h":
            print(f"Usage: {sys.argv[0]} [OPTION]")
            print("Fetches the forwarded port from a running Gluetun LXC container and")
            print("dynamically updates the qBittorrent LXC container's configuration to use this port.")
            print("")
...
def readConfigFile():
    ...
    #check if there is a config file in /etc (common config location for system services) 
    if os.path.isfile("/etc/port_update.conf"):
        config.read("/etc/port_update.conf")
        return config
...
def check_config():
    ...
    config = readConfigFile()
    if config: 
        sections = ['paths', 'lxc', 'gluetun', 'logging']
...
def update_port():
    ...
    if envvars: 
        ...
        #checking if path to gluetun forwarded port is set, if not fetch ip and port
        try: 
        ...
        try:                                  
            container_id = config["lxc"]["container_id"]
        except: 
            log("ERROR", f"Unable to get container_id from port_update.conf")
            exit()
    ...
    # For LXC, we need to read the file from inside the container
    try:
        result = subprocess.run(['pct', 'exec', container_id, '--', 'cat', qbt_path], 
                               capture_output=True, text=True, check=True)
        lines = result.stdout.splitlines(keepends=True)
    except subprocess.CalledProcessError as e:
        log("error", f"Unable to read qBittorrent config from container {container_id}: {str(e)}")
        exit()
    ...
    if update == True: 
        lxc_qbittorrent("stop", container_id)
        try:        
            # Write the updated config back to the container
            updated_content = ''.join(updated_lines)
            result = subprocess.run(['pct', 'exec', container_id, '--', 'sh', '-c', f'cat > {qbt_path}'], 
                                   input=updated_content, text=True, check=True)
        ...
        lxc_qbittorrent("start", container_id) 
...
#a function that stops, starts or restarts LXC containers using pct
def lxc_qbittorrent(action, container_id):
    try:
        match action: 
            case "stop":
                log("info", f"Stopping LXC container {container_id}") 
                subprocess.run(['pct', 'stop', container_id], check=True)
                log("info", f"Container successfully stopped!")
            case "start":
                log("info", f"Starting LXC container {container_id}") 
                subprocess.run(['pct', 'start', container_id], check=True)
                log("info", f"Container successfully started!")
            case "restart":
                log("info", f"Restarting LXC container {container_id}") 
                subprocess.run(['pct', 'stop', container_id], check=True)
                subprocess.run(['pct', 'start', container_id], check=True)
        ...
        log("error", f"Error {action}ing LXC container {container_id}: {str(e)}")
        exit()    
